...
input_mode.add_argument('--tcp', metavar = 'TCP_HOST_AND_PORT', default = 'localhost:43555', help = 'Use a remote DIAG device connected through TCP.')
...
if args.dlf_read:
    ...
    if not port.isdigit():
        exit('--tcp argument must be of the form HOST:PORT')
    port = int(port)
    diag_input = TcpConnector(host, port)
elif args.usb_modem:
